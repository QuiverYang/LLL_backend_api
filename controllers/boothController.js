...
const sendForgetPswEmail = async(req, res, next) => {
    ...
    let to = email;
    let subject = 'Lead Long Line(展覽端)忘記密碼通知(送出時間：' + new Date().toLocaleString() + ')';
    let text = '敬愛的Lead Long Line(展覽端)用戶您好：\n\n'
    ...
    emailController.sendSystemEmail(to, subject, text, html);
    sendVerifyNumTime = new Date().getTime();
    res.json({status: 200, verifyNum: verifyNum, sendVerifyNumTime: sendVerifyNumTime, serverMsg: '200, ok, send forget psw verify email success.', clientMsg: '驗證碼已發送至' + email});
}
